...
def swap_about_section(html, leader):
    ...
    if leader.get("photo_path"):
        ...
        # Some leaders write their own bio starting with a self-introduction
        # ("Hi, I'm ...", "Hello, I'm ...", "Hi there, I'm ..."). Adding our
        # own "Meet X." lead-in on top of that reads as a redundant double
        # introduction, so skip it in that case and let their own opening
        # stand on its own.
        already_self_introduces = bool(re.match(r"^\s*(hi|hello|hey)[,!]?\s+(there[,!]?\s+)?i'?m\s", bio, re.IGNORECASE))
        lead_in = "" if already_self_introduces else f"<strong>Meet {leader['name']}.</strong> "
